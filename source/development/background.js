...
async function saveCompletedMeeting() {
  ...
  const titleResult = await chrome.storage.local.get({ noteTitle: null });
  const payload = {
    externalId: activeMeeting?.id || crypto.randomUUID(),
    userId: await ensureUserId(),
    title: titleResult.noteTitle == null ? null : String(titleResult.noteTitle).trim().slice(0, 160) || null,
...
async function saveAndAnalyzeFromText({ transcript, title, platform = 'Manual' } = {}) {
  ...
  const payload = {
    ...
    userId: await ensureUserId(),
    title: title == null ? null : String(title).trim().slice(0, 160) || null,
...
async function startRecordingInternal({ automatic = false, tabCaptureStreamId = null, captureMeetingAudio = false } = {}) {
  ...
  await setInterim('');
  await chrome.storage.local.set({ noteTitle: activeMeeting.title || null });
  await setRecordingState({
  ...
  (async () => {
    try {
      switch (message.type) {
        ...
        case 'SET_TITLE': {
          const title = message.title == null ? null : (String(message.title).trim().slice(0, 120) || null);
