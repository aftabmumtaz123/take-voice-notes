...
btnNewNote.addEventListener('click', async () => {
  ...
  if (result?.ok) {
    ...
    sessionTitle.value = '';
    await chrome.runtime.sendMessage({ target: 'background', type: 'SET_TITLE', title: null });
...
btnOpenMeeting?.addEventListener('click', async () => {
  ...
  await chrome.tabs.create({ url: `https://take-voice-notes.vercel.app/meetings/${encodeURIComponent(lastCompletedMeetingId)}` });
});
...
dashboardLink.addEventListener('click', async (e) => {
  e.preventDefault();
  try {
    await chrome.tabs.create({ url: 'https://take-voice-notes.vercel.app/' });
  } catch (error) {
...
btnSaveApiKey?.addEventListener('click', async () => {
  ...
  const serverUrl = (authServerUrl?.value || '').trim().replace(/\/$/, '') || 'https://take-voice-notes.vercel.app';
  if (!apiKey) {
...
btnOpenWebAuth?.addEventListener('click', async () => {
  ...
  if (!result?.ok) {
    showAuthError(result?.error || 'Could not open https://take-voice-notes.vercel.app — is the server running?');
  }
});
